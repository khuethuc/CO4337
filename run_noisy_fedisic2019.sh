#!/usr/bin/env bash
# Noisy experiments — FedISIC-2019  (Dirichlet label-noise attack)
...
#
# Outputs: outputs_noisy_fedisic2019/seed_{SEED}/{run_id}/
...
for SEED in 42 123 321; do
    OUTDIR="outputs_noisy_fedisic2019/seed_${SEED}"
    printf '\n\n######################################################################\n'
    printf '##  SEED = %s  |  FedISIC-2019  |  Noisy (Dirichlet 20%%)\n' "$SEED"
    printf '######################################################################\n'
