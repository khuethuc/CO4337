...
from dataloader_dirichlet import *
...
parser.add_argument('--dirichlet_alpha', default=0.3, type=float,
                    help='Dirichlet alpha (smaller => more skewed)')
parser.add_argument('--dirichlet_min_size', default=10, type=int,
                    help='minimum samples per client for Dirichlet partition')

parser.add_argument('--dirichlet_attack', action='store_true',
                    help='enable Dirichlet attack: re-partition only malicious clients with attack_alpha')
parser.add_argument('--adv_ratio', default=0.0, type=float,
                    help='fraction of malicious clients (0~1)')
parser.add_argument('--adv_contiguous', action='store_true',
                    help='choose malicious clients as a contiguous segment on the ring')
parser.add_argument('--adv_start', default=0, type=int,
...
def run(rank, size):
    ...
    train_loader, bsz_train = partition_trainDataset(
        ...
        min_size=args.dirichlet_min_size,
        dirichlet_attack=args.dirichlet_attack,
        adv_ratio=args.adv_ratio,
        adv_contiguous=args.adv_contiguous,
   ...
   # Check non-iid distribution
    ...
    for epoch in range(0, args.epochs):  
        ...
        dt, prec1, loss, m = train(train_loader, model, criterion, optimizer, epoch, 
                                bsz_train, optimizer.param_groups[0]['lr'], 
                                device, receiver, sender, min_len=min_len, gpu_index=rank,
...
def train(train_loader, model, criterion, optimizer, epoch, batch_size, lr, device, receiver=None, sender=None, min_len = None, gpu_index: int = 0, monitor_every: int = 50):
    """
        Run one train epoch
    """
